    ...
    return (
      <div className="flex items-center gap-3">
        <button
          ...
          className="eyebrow text-faint transition-colors hover:text-sconce"
        >
          Rename this cinema
        </button>
        {name && (
          <button
            ...
            className="eyebrow text-faint transition-colors hover:text-rose"
          >
            Remove name
          </button>
        )}
  ...
  return (
    ...
    >
      ...
      <input
        ...
        maxLength={120}
        autoFocus
        className="min-w-0 flex-1 border border-edge bg-velvet/50 px-3 py-2 text-sm text-paper placeholder:text-faint focus:border-sconce focus:outline-none sm:max-w-xs"
      />
      <button
        type="submit"
        disabled={saving}
        className="border border-sconce/60 px-3.5 py-2 text-[13px] text-sconce transition-colors hover:bg-sconce hover:text-ink disabled:opacity-50"
      >
        {saving ? "Saving…" : "Save"}
      </button>
      {name !== null && (
        <button
          ...
          className="px-2 py-2 text-[13px] text-faint transition-colors hover:text-paper"
        >
          Cancel
        </button>
      )}
      {error && <p role="alert" className="w-full text-[12px] text-rose">{error}</p>}
    </form>
  );
}
